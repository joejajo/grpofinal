...
v11 changes vs v10:
  ...
  - Default lora_r raised to 32 for better expressiveness on math tasks
  - Length-ramp penalty threshold increased 700 -> 1200 chars (less punishing on verbose reasoning)
  - Bootstrap phase extended 0-149 (was 0-99); hard phase starts at 600 (was 500)
  - Bootstrap labeled-fallback caps raised so the model has more room to score before format lock-in
  - MIN_THINK_CHARS default lowered 120 -> 80 (easier to earn reasoning bonus early)
  - REASONING_KEYWORDS extended with more root-solving vocabulary
  - System prompt rewritten: clearer format contract, explicit single-boxed-line instruction
  - Default generation temperature raised 0.35 -> 0.70 (diversity is critical for GRPO signal)
  - Default vllm_gpu_memory_utilization raised 0.30 -> 0.45
  - GRPOConfig adds dataloader_num_workers=0 (avoids tokenizer fork hang on some HPC setups)
  - Beta default 0.0 -> 0.001 (small KL penalty prevents catastrophic policy drift)
  - eval_every_steps default 100 (was 100) - SLURM now enables it at 150
...
def main():
    ...
    ap.add_argument("--beta", type=float, default=0.001)  # v11: 0.0->0.001 for policy stability
    ...
    ap.add_argument("--temperature", type=float, default=0.70)  # v11: 0.35->0.70 for GRPO diversity
    ...
    ap.add_argument("--lora_alpha", type=int, default=64)  # v11: 32->64 (keep ratio 2:1)
