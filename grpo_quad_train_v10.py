...
"""
GRPO (Countdown/R1-style) for Quadratic Integer Roots
...
Prompt design:
  Output format is intentionally simple:
    \boxed{r1, r2}
  No XML tags are required.

Single combined reward in [0,1]:
  - math (0.0 / 0.4 / 0.8) via substitution
  - small shaping via Vieta (sum/product) capped inside math
  - format bonus mostly gated by correctness (prevents farming format with wrong roots)
  - small reasoning bonus only when math is at least partially correct
  - penalties for XML-like tags, multiple boxed pairs, and trailing junk

Debug:
  prints FULL completion(s) for N generations every K reward calls
  includes step/epoch, equation, (a,b,c), GT, parsed roots, residuals, and effective message.
"""
...
MIN_THINK_CHARS = 120        # minimum reasoning characters before the final boxed answer
REASONING_KEYWORDS = (
    "discriminant",
    "delta",
    "sqrt",
...
def combined_reward_func(
    ...
    **kwargs
    ...
    bootstrap_phase = step_now < 100
    strict_phase = step_now >= 100
    hard_phase = step_now >= 500
...
def main():
    ...
    ap.add_argument("--vllm_gpu_memory_utilization", type=float, default=0.30)
    ...
    # Reasoning control
    ap.add_argument("--min_think_chars", type=int, default=120)
    ...
    # Simple, parse-stable format contract for both reward and eval.
    SYSTEM_PROMPT = (
        "You are a math solver.\n"
        "Show brief reasoning first (2-5 short lines), then output exactly one final line in this format:\n"
        "\\boxed{r1, r2}\n"
        "Rules:\n"
        "- r1 and r2 must be integers\n"
        "- r1 <= r2\n"
        "- do not use XML/HTML tags\n"
        "- reasoning must appear before the final boxed line\n"
        "- no extra text after the boxed answer\n"
    )
